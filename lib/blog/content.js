 ...
 */
export function resolveSectionImage(sectionText, postSlug, postTitle) {
 ...
 */
export function getPostImageGallery(postSlug, postTitle) {
  const discovered = getDiscoveredBlogImages();
  const gallery = [];
  const seen = new Set();
  ...
  add(resolveBlogImage(postSlug, postTitle));
 ...
 */
export function prepareArticleSections(sections, postSlug, postTitle, heroImage) {
  const gallery = getPostImageGallery(postSlug, postTitle);
  ...
  for (const section of sections) {
    if (section.type === "heading") {
      ...
      if (level === 2) {
        let figureSrc =
          resolveSectionImage(section.text, postSlug, postTitle) ||
...
export function preparePost(post) {
  const image = resolveBlogImage(post.slug, post.title);
