...
export function TemplateFloral({ data, theme, headingFont, bodyFont }) {
  ...
  return (
    ...
    >
      ...
      <div className={`p-6 flex gap-6 items-center bg-slate-50/50 ${hasPhoto ? "flex-row-reverse" : "flex-col text-center"}`}>

        {hasPhoto && (
          <img src={photoDataUrl} alt="Profile" className="w-24 h-32 rounded-lg object-cover overflow-hidden" style={{ borderColor: accent }} />
        )}

        <div className={hasPhoto ? "flex-1 text-right" : "flex-1 text-center"}>
